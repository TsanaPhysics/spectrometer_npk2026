...
**ห้องปฏิบัติการวิจัย:** กลุ่มวิจัยฟิสิกส์เกษตรดิจิทัลและปัญญาประดิษฐ์ (Digital Agriphysics & AI Research Lab)  
...
* **สมการคำนวณย้อนกลับหาความเข้มข้น (Exact Mathematical Inversion):**
  $$aC^2 + bC + (c - A) = 0$$
  เนื่องจาก $a < 0$ (พาราโบลาคว่ำ) และมีจุดยอดอยู่ที่ $C_{\text{vertex}} = -\frac{b}{2a} \approx 8.51\text{ mg/L}$ รากทางกายภาพในช่วง $1.0 - 10.0\text{ mg/L}$ คือ:
  $$C = \frac{-b - \sqrt{b^2 - 4a(c - A)}}{2a}$$
* **ผลลัพธ์การประเมิน:**
  * **$R^2_{\text{Abs}}$:** **$\mathbf{0.9925}$** (ความแม่นยำสูงสุด เพิ่มขึ้นจากเส้นตรงถึง $+14.02\%$)
  * **$\text{RMSE}_{\text{Abs}}$:** **$0.01345\text{ Abs}$** (ลดทอนความคลาดเคลื่อนลงถึง $77.4\%$)
  * **$\text{MAE}_{\text{Abs}}$:** **$0.01136\text{ Abs}$**
  * **$\text{RMSE}_{\text{Conc}}$:** **$1.264\text{ mg/L}$**
  * **Inference Latency บน ESP32:** **$1.8\ \mu\text{s}$** (ประมวลผลเร็วที่สุด ใช้ฟังก์ชัน `sqrtf()` ระดับฮาร์ดแวร์ FPU)
...
1. `PAGE_DASHBOARD` (1/8): ภาพรวมระบบ อุณหภูมิสี แสงสว่าง และสถานะเซนเซอร์
2. `PAGE_NITROGEN` (2/8): โหมดวิเคราะห์ไนโตรเจนเฉพาะหลอด (3-Tier Auto-Switching $0.1 - 100\text{ mg/L}$)
3. `PAGE_PHOSPHORUS` (3/8): โหมดวิเคราะห์ฟอสฟอรัสเฉพาะหลอด (Q1 Quadratic Inversion $R^2 = 0.9925$)
4. `PAGE_POTASSIUM` (4/8): โหมดวิเคราะห์โพแทสเซียมเฉพาะหลอด (Turbidimetry 625nm)
5. `PAGE_SOIL_PH` (5/8): โหมดวิเคราะห์ความเป็นกรด-ด่างดิน (Ratiometric Optical Pre-Model)
6. `PAGE_NPK_METER` (6/8): หน้าจอรวมสรุปค่า N, P, K และ pH ดิน
7. `PAGE_SPECTRUM` (7/8): กราฟสเปกตรัมการดูดกลืนแสง 5 แถบคลื่น (465, 500, 525, 590, 625 nm)
8. `PAGE_CALIBRATE` (8/8): ระบบสอบเทียบมาตรฐานหลายจุดประจำแปลง (In-Situ Standard Curve Wizard)
