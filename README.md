...
1. **Page 1: System Dashboard (`[1/8]`):**
   ...
   - แบรนด์ระดับพรีเมียม **`SpecJC +AI Analyzer`** ขนาดใหญ่พิเศษ (TextSize 2, 4-Point Bold) ด้วยคู่สีเฉพาะ: **`Spec` สีฟ้าสด (`0x07FF`)**, **`JC` สีแดงสด (`TFT_RED`)**, **`+AI` สีเหลืองทอง (`TFT_YELLOW`)**, และ **`Analyzer` สีเขียวสดใส (`TFT_GREEN`)** จัดวางเต็มความกว้างอย่างสวยงามและสมดุลบนแถบแบนเนอร์ด้านบน
